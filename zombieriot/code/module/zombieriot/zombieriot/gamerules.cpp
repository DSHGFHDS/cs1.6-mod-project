...
void *(*g_pfnInstallGameRules)(void);
JMPHook *g_phInstallGameRules;
DWORD g_dwCheckWinConditions;
BYTE g_bCheckWinConditionsRestoreByte;

void *InstallGameRules(void)
{
	g_phInstallGameRules->Detach();
	g_pGameRules = (CHalfLifeMultiplay *)g_pfnInstallGameRules();

	if (!g_dwCheckWinConditions)
	{
		void **vtfuncs = *(void ***)g_pGameRules;
		g_dwCheckWinConditions = (DWORD)vtfuncs[65];
		g_bCheckWinConditionsRestoreByte = *(BYTE *)g_dwCheckWinConditions;
	}
...
{
	if (noend)
		UTIL_WriteMemoryBYTE((void *)g_dwCheckWinConditions, 0xC3);
	else
		UTIL_WriteMemoryBYTE((void *)g_dwCheckWinConditions, g_bCheckWinConditionsRestoreByte);
}
...
{
	if (team == TEAM_TERRORIST)
		g_pGameRules->m_iNumTerroristWins = score;
	else if (team == TEAM_CT)
		g_pGameRules->m_iNumCTWins = score;
}
